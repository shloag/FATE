...
class CKKSTensor(object):
    def __init__(self, obj, partitions=1):
        if obj is None:
            raise ValueError("Cannot convert None to CKKS tensor")
        ...
        if isinstance(obj, (list, np.ndarray)):
            ...
            raise ValueError(f"Cannot convert obj to CKKS tensor, object type is {type(obj)}")

        LOGGER.debug("tensor's partition is {}".format(self._partitions))

    def __add__(self, other):
        if isinstance(other, CKKSTensor):
            return CKKSTensor(self._obj.join(other._obj, lambda v1, v2: v1 + v2))
        elif isinstance(other, CTableABC):
            return CKKSTensor(self._obj.join(other, lambda v1, v2: v1 + v2))
        elif isinstance(other, (np.ndarray, int, float)):
            return CKKSTensor(self._obj.mapValues(lambda v: v + other))
        else:
            raise ValueError(f"Unrecognized type {type(other)}, does not support addition")
    ...
    def __sub__(self, other):
        if isinstance(other, CKKSTensor):
            ...
            raise ValueError(f"Unrecognized type {type(other)}, does not support subtraction")
    ...
    def __mul__(self, other):
        if isinstance(other, (int, float)):
            return CKKSTensor(self._obj.mapValues(lambda val: val * other))
        elif isinstance(other, np.ndarray):
            return CKKSTensor(self._obj.mapValues(lambda val: np.matmul(val, other)))
        elif isinstance(other, CTableABC):
            other = CKKSTensor(other)
            return self.__mul__(other)
        elif isinstance(other, CKKSTensor):
            ret = self.numpy() * other.numpy()
            return CKKSTensor(ret, partitions=max(self.partitions, other.partitions))
    ...
    def multiply(self, other):
        if isinstance(other, np.ndarray):
            ...
            rhs = CKKSTensor(other)
            return CKKSTensor(self.multiply(rhs))
        elif isinstance(other, CTableABC):
            other = CKKSTensor(other)
    ...
    def T(self):
        ...
        return CKKSTensor(new_data, self.partitions)
    ...
    def encrypt(self, encrypt_tool):
        return CKKSTensor(encrypt_tool.distribute_encrypt(self._obj))

    def decrypt(self, decrypt_tool):
        return CKKSTensor(self._obj.mapValues(lambda val: decrypt_tool.recursive_decrypt(val)))

    def encode(self, encoder):
        return CKKSTensor(self._obj.mapValues(lambda val: encoder.encode(val)))

    def decode(self, decoder):
        return CKKSTensor(self._obj.mapValues(lambda val: decoder.decode(val)))
    ...
    def fast_matmul_2d(self, other):
        ...
        """
        if isinstance(other, np.ndarray):
            mat_tensor = CKKSTensor(other, partitions=self.partitions)
    ...
    def matmul_3d(self, other, multiply='left'):
        ...
        if isinstance(other, CKKSTensor):
            ...
            mat = CKKSTensor(other)
        elif isinstance(other, np.ndarray):
            mat = CKKSTensor(other, partitions=self.partitions)
        else:
            raise ValueError('only support numpy array and CKKS Tensor')

        if multiply == 'left':
            return CKKSTensor(self._obj.join(mat._obj, lambda val1, val2: np.tensordot(val1, val2, (1, 0))),
                                  partitions=self._partitions)

        if multiply == 'right':
            return CKKSTensor(mat._obj.join(self._obj, lambda val1, val2: np.tensordot(val1, val2, (1, 0))),
                                  partitions=self._partitions)
    ...
    def element_wise_product(self, other):
        if isinstance(other, np.ndarray):
            ...
            mat = other
        return CKKSTensor(self._obj.join(mat._obj, lambda val1, val2: val1 * val2))

    def squeeze(self, axis):
        if axis == 0:
            return CKKSTensor(list(self._obj.collect())[0][1], partitions=self.partitions)
        else:
            return CKKSTensor(self._obj.mapValues(lambda val: np.squeeze(val, axis=axis - 1)))

    def select_columns(self, select_table):
        return CKKSTensor(self._obj.join(select_table, lambda v1, v2: v1[v2]))
